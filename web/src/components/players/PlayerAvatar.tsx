...
interface PlayerAvatarProps {
  ...
  size?: 'sm' | 'md' | 'lg' | 'xl'
  /** Show the jersey number as a teal badge at bottom-right */
    ...
    <div className={cn('relative shrink-0 inline-block', className)}>
      ...
      {imgSrc ? (
        <img
          ...
          className={cn(
            'rounded-full object-cover object-center bg-surface-high',
            s.outer,
          )}
        />
      ) : (
        <div
          className={cn(
            'rounded-full bg-surface-highest flex items-center justify-center font-display font-black text-orange',
            s.outer,
            s.text,
          )}
        >
      ...
      {/* Jersey number badge (teal, bottom-right) */}
      {showJerseyBadge && player.jersey != null && (
        <div
          className={cn(
            'absolute rounded-full bg-secondary-container text-surface flex items-center justify-center font-display font-black leading-none ring-2 ring-background',
            s.badge,
          )}
        >
          {player.jersey}
